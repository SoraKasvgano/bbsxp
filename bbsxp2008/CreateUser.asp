...
if SiteConfig("AccountActivation")=2 then
	...
	if Execute("Select UserName from ["&TablePrefix&"UserActivation] where ActivationKey='"&SqlString(ActivationKey)&"' and Email='"&SqlString(UserEmail)&"'").eof then error("邀请码错误！")
...
if Request.Form("menu")="AddUserName" then
	...
	if SiteConfig("AccountActivation")=2 then Execute("Delete from ["&TablePrefix&"UserActivation] where Email='"&SqlString(UserEmail)&"'")
...
<%	end if%>
	<tr class="CommonListCell">
		<td align="right" valign="middle" width="23%"><b>您的Email地址：</b><br /><%if SiteConfig("AccountActivation")=1 then%><font color="FF0000">密码将通过Email发送</font><%end if%></td>
	 	<td align="Left" valign="middle" width="77%">
		<input type="text" name="UserEmail" size="40" onblur="CheckMail(this.value)" <%if SiteConfig("AccountActivation")=2 then%> value="<%=Server.HTMLEncode(UserEmail)%>" readonly<%end if%> /> <span id="CheckMail" style="color:#FF0000"></span></td>
	</tr>
