...
if not Execute("select * from ["&TablePrefix&"Reputation] where CommentFor='"&SqlString(CommentFor)&"' and IPAddress='"&SqlString(REMOTE_ADDR)&"' and DateDiff("&SqlChar&"d"&SqlChar&",DateCreated,"&SqlNowString&")=0").eof then AlertForModal("此IP今天已对 "&CommentFor&" 评价过！")
...
if BestRole<>1 then
	...
	if IsArray(CommentByGetRows) then
		For i=0 To Ubound(CommentByGetRows,2)
			if CommentByGetRows(0,i)=CommentFor then  AlertForModal("再次对 "&CommentFor&" 进行声望评价之前，您必须对其他"&SiteConfig("ReputationRepeat")&"个用户进行声望评价！")
		Next
	End if
...
if Request_Method = "POST" then
	...
	if len(Comment)>500 then AlertForModal("评论内容不能超过500个字符！")
	...
	AddApplication "Message_"&CommentFor,"【系统讯息】<a target=_blank href=Profile.asp?UserName="&Server.URLEncode(CommentFor)&">"&Server.HTMLEncode(CookieUserName)&" 对您进行了声望评价</a>"
...
<%
	...
	Response.clear
%>
<title>对 <%=Server.HTMLEncode(CommentFor)%> 进行评论</title>
