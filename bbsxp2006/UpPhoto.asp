...
if FileSize > int(SiteSettings("MaxPhotoSize")) then error("<li>图片大小超过限制 "&CheckSize(SiteSettings("MaxPhotoSize"))&"<br>当前图片大小为 "&CheckSize(FileSize)&"")
...
if FileExt<>"gif" and FileExt<>"jpg" then error("<li>上传图片只允许 GIF 或 JPG 格式")
if FileSize < 0 then error2("当前文件为空文件")
if FileSize > int(SiteSettings("MaxPhotoSize")) then error2("文件大小超过限制 "&CheckSize(SiteSettings("MaxPhotoSize"))&"\n当前文件大小为 "&CheckSize(FileSize)&"")
...
Conn.execute("update [BBSXP_Users] set Userphoto='UpFile/UpPhoto/"&id&"."&FileExt&"' where UserName='"&SqlString(CookieUserName)&"'")
Message=Message&"<li>照片上传成功<li><a href=UpPhoto.asp>继续上传照片</a><li><a href=Default.asp>返回论坛首页</a>"
succeed(""&Message&"<meta http-equiv=refresh content=3;url=UpPhoto.asp>")

elseif Request("menu")="custom" then
RawUserphoto=Trim(Request.Form("Userphoto"))
Userphoto=SafeUrl(RawUserphoto)
if RawUserphoto="" then error("<li>照片URL地址没有填写")
if Userphoto="" then error2("URL地址非法")

Conn.execute("update [BBSXP_Users] set Userphoto='"&HTMLEncode(Userphoto)&"' where UserName='"&SqlString(CookieUserName)&"'")
End If

CurrentUserPhoto=SafeUrl(Conn.Execute("Select Userphoto From [BBSXP_Users] where UserName='"&SqlString(CookieUserName)&"'")(0))
%>
...
<table width=100% border=0 cellpadding="0" cellspacing="1" class=a2 height="218" align="center"><tr>
	<td class=a1 height="25" width="70%"><p align=center>点击浏览按钮选择要上传的照片图片</p>
	</td><td class=a1><div align=center>您目前的照片</div></td>
</tr><tr><td class=a3 width="70%"><center>
<input type=file name="file" size=33><br><br><input type=submit value=" 确 定 "></form></td>
		<td rowspan=3 class=a3 align="center"><%if CurrentUserPhoto<>"" then%><img src="<%=CurrentUserPhoto%>" onload='javascript:if(this.width>200)this.width=200'><%end if%></td>
</tr><tr><td class=a1 height="25" width="70%"><center>自定义照片</td><tr>
<td class=a3 width="70%"><form method=Post action=UpPhoto.asp?menu=custom><center>照片URL：<input size="35" value="<%=CurrentUserPhoto%>" name="Userphoto">
