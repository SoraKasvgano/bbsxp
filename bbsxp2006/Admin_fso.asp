...
TableSuffix=SafeTableSuffix(Request("tablename"))
if TableSuffix="" then error2("数据表名非法")
SQL="sp_changeobjectowner '[BBSXP_Posts"&TableSuffix&"]','dbo'"
Conn.execute(SQL)
error2("已经成功把表的所有者更改为 dbo")

case "CreatTable"
TableSuffix=SafeTableSuffix(Request("tablename"))
if TableSuffix="" then error2("数据表名非法")

Sql="CREATE TABLE [BBSXP_Posts"&TableSuffix&"] ("&_
...
Sql="ALTER TABLE [BBSXP_Posts"&TableSuffix&"] ADD CONSTRAINT [FK_BBSXP_Posts"&TableSuffix&"_BBSXP_Threads] FOREIGN KEY ([ThreadID]) REFERENCES [BBSXP_Threads] ([Id]) ON DELETE CASCADE  ON UPDATE CASCADE"
Conn.execute(sql)

Conn.execute("update [BBSXP_SiteSettings] set DefaultPostsName="&TableSuffix)
error2("建表成功")

case "update"
TableSuffix=SafeTableSuffix(Request("TableName"))
if TableSuffix = "" Then ThisValue = "Null" Else ThisValue = TableSuffix
Conn.execute("update [BBSXP_SiteSettings] set DefaultPostsName="&ThisValue)
error2("设置成功")

case "Del"
TableSuffix=SafeTableSuffix(Request("tablename"))
if TableSuffix="" then error2("系统默认的数据表不能删除")
If not Conn.Execute("Select PostsTableName From [BBSXP_Threads] where PostsTableName='"&SqlString(TableSuffix)&"'" ).eof Then error2("该表中有对应的主题，不能删除")
if ""&SiteSettings("DefaultPostsName")&""=""&TableSuffix&"" then error2("当前正在使用中的数据表不能删除")
Conn.Execute("drop table [BBSXP_Posts"&TableSuffix&"]")
error2("删除成功")
...
ho=""&ho&""
if ho<>"" and SafeFileName(ho)=ho and instr(ho,"..")=0 then
...
if BakDbPath="" then error2("备份数据库路径非法")
set MyFileObject=Server.CreateOBject("Scripting.FileSystemObject")
MyFileObject.CopyFile ""&Server.MapPath(DB)&"",""&Server.MapPath(BakDbPath)&""
error2("备份成功")

case "bakhf"
BakDbPath=SafeDbRelativePath(Request.Form("BakDbPath"))
if BakDbPath="" then error2("恢复数据库路径非法")
set MyFileObject=Server.CreateOBject("Scripting.FileSystemObject")
MyFileObject.CopyFile ""&Server.MapPath(BakDbPath)&"",""&Server.MapPath(DB)&""
error2("恢复成功")
...
sub bak
If IsSqlDataBase<>0 Then error2("SQL 版本无法操作 ACCESS 数据库")
%>
...
<table cellpadding="0" cellspacing="0" style="border-collapse: collapse" width="90%">
<tr>
<td width="30%" height="25">备份数据库路径 </td>
<td width="70%">
<input size="30" value="database/backup(<%=SafeDateSuffix()%>).mdb" name="BakDbPath"></td>
</tr>
<tr>
<td width="30%" height="25">数据库路径</td>
<td width="70%">已隐藏</td>
...
<td width="70%">
<input type="hidden" value="__current__" name="dbpath">当前 ACCESS 数据库（路径已隐藏）</td>
</tr>
<tr>
...
<table cellspacing="1" cellpadding="2" width="70%" border="0" class="a2" align="center" height="130">
  <tr height=25>
    <td class=a1 align=middle colspan=3>统计所占空间</td>
  </tr>
   <tr height=25>
    <td class=a3 width="29%">上传头像所占空间<br></td>
    <td class=a3 width="60%"><IMG height=10 src="images/bar/0.gif" width=<%=Int(UpFacesize/totalBytes*100)%>%> </td>
    <td class=a3><%=CheckSize(UpFacesize)%></td></tr>
   <tr height=25>
    <td class=a3 width="29%">上传照片所占空间</td>
    <td class=a3 width="60%"><IMG height=10 src="images/bar/1.gif" width=<%=Int(UpPhotosize/totalBytes*100)%>%> </td>
    <td class=a3><%=CheckSize(UpPhotosize)%>字</td></tr>
   <tr height=25>
    <td class=a3 width="29%">上传附件所占空间</td>
    <td class=a3 width="60%"><IMG height=10 src="images/bar/2.gif" width=<%=Int(UpFilesize/totalBytes*100)%>%> </td>
    <td class=a3><%=CheckSize(UpFilesize)%>字</td></tr>
   <tr height=25>
    <td class=a3 width="29%" height="24">BBSXP目录总共所占空间</td>
    <td class=a3 width="60%" height="24"><IMG height=10 src="images/bar/4.gif" width=<%=Int(tolsize/totalBytes*100)%>%> </td>
    <td class=a3><%=CheckSize(tolsize)%>字</td></tr></table>
...
<a href="?menu=files&dir=<%=Server.URLEncode(dir)%>"><%=HTMLEncode(dir)%>目录下</a>共 <font color="FF0000"><b><%=count%></b></font> 个文件
...
%>
<img src=images/folder.gif> <a href="?menu=files&dir=<%=Server.URLEncode(theFolder.Name)%>"><%=HTMLEncode(theFolder.Name)%></a><br>
<%
next
%>
...
<input type=hidden name=dir value="<%=HTMLEncode(dir)%>">
...
<input type=checkbox name=chkall onclick=CheckAll(this.form) value="ON"></td>
  ...
  <td width=128 align=center>大小</td><td width=140 align="center">类型</td><td align="center">修改时间</td></tr>
<%
Pagesize=20
...
response.write "<tr class=a3><td align=center><input type='CheckBox' value='"&HTMLEncode(thing.name)&"' name=files></td><td align=center><a target=_blank href='UpFile/"&dir&"/"&Server.URLEncode(thing.Name)&"'>"&HTMLEncode(thing.Name)&"</a></td><td align=center>" &CheckSize(thing.size)& "</td><td align=center>" & HTMLEncode(thing.type) & "</td><td align=center>" & cstr(thing.datelastmodified) & "</td></tr>"
...
<tr><td>
<INPUT type=submit onclick="checkclick('您确定要删除所选文件吗?');" value=" 删 除 ">
...
<input type=hidden name=dir value="<%=HTMLEncode(dir)%>">
搜索文件名称
<input size="20" name="genre" value="<%=HTMLEncode(genre)%>">
...
<%
if Page>1 then
	response.write "<a href=?menu=files&dir="&dir&"&genre="&Server.URLEncode(genre)&"&Page=1>首页</a>&nbsp;&nbsp;<a href=?menu=files&dir="&dir&"&genre="&Server.URLEncode(genre)&"&Page="& Page-1 &">上一页</a>&nbsp;&nbsp;"
else
	response.write "首页&nbsp;&nbsp;上一页&nbsp;&nbsp;"
end if
if Page<i/Pagesize then
	response.write "<a href=?menu=files&dir="&dir&"&genre="&Server.URLEncode(genre)&"&Page="& Page+1 &">下一页</a>&nbsp;&nbsp;<a href=?menu=files&dir="&dir&"&genre="&Server.URLEncode(genre)&"&Page="& Pagenum &">尾页</a>"
else
...
<table  cellspacing="1" cellpadding="2" border="0" width="100%" class="a2">
	<tr class=a1>
		<td align="center" colspan="8" height="25"><%=rs(0)%>数据库表管理</td>
	</tr>
	<tr class=a3>
...
%>
	...
	<tr class=a3>
		<td colspan="8">
		<i>说明：</i>选中的数据表为当前论坛使用的数据表。<br>
		<i>建议：</i>ACCESS 版本超过 200,000 条数据建议分表；MSSQL 版本超过 500,000 条数据建议分表。</td>
	</tr>
</table></form>
<br>
...
<table  cellspacing="1" cellpadding="2" border="0" width="100%" class="a2">
	<tr class=a1>
		<td align="center" colspan="2" height="25">新增数据表</td>
	</tr>
	<tr class=a3>
...
Select Case order
Case "id","FileName","ContentSize","ContentType","Created","UserName","TotalDownloads","Description"
Case Else
order="id"
End Select

if Search<>"" then SearchSql="where "&Search&" like '%"&SqlString(Key)&"%'"

sql="[BBSXP_PostAttachments] "&SearchSql&""
rs.Open ""&sql&" order by "&order&" Desc",Conn,1
TotalCount=conn.Execute("Select count(ID) From "&sql&"")(0)
PageSetup=20
...
%>
附件总数 <font color="#FF0000"><b><%=TotalCount%></b></font> 个
...
<option value="Description">关联描述</option>
</select> 包含
